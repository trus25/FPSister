...
def client_init():
    ...
    _role = input(">> Choose role(1 for player or 2 for spectator): ")

    return _name, _role, _address


def connect(_name, _role, _address):
    _uri = "PYRONAME:game_server@" + _address + ":7777"
    _server = Pyro4.Proxy(_uri)
    print("test")
    print(_server.register_client(_name, _role).get("message"))
    return _uri, _server
...
if __name__ == "__main__":
    ...
    name, role, address = client_init()

    uri, server = connect(name, role, address)
